...
class StockCardRepository
{
    ...
    {
        $stockCard = StockCard::create($data);

        // If caller provided explicit timestamps, apply them to avoid using now()
        if (! empty($data['created_at'])) {
            $stockCard->timestamps = false;
            $stockCard->created_at = Carbon::parse($data['created_at']);
            $stockCard->updated_at = ! empty($data['updated_at']) ? Carbon::parse($data['updated_at']) : $stockCard->created_at;
            $stockCard->save();
            $stockCard->timestamps = true;
        }
