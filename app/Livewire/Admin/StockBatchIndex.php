...
class StockBatchIndex extends Component
{
    ...
    // Product pagination per page (for quick add modal pagination)
    public int $productPerPage = 10;

    public int $productPage = 1;

    // Stock summary tab (for batch listing summary cards)
    public string $stockSummaryTab = 'product'; // 'product' or 'location'

    // Summary date filter properties (for stock summary breakdown cards)
    ...
    {
        // Reset product page when per page changes
        $this->productPage = 1;
    }

    public function setStockSummaryTab($tab)
    {
        // Set active summary tab (product or location)
        if (in_array($tab, ['product', 'location'])) {
    ...
    #[Computed]
    public function editFilteredSubcategories()
    {
        if (! $this->editCategoryId) {
            return Subcategory::all();
        }

        return Subcategory::where('category_id', $this->editCategoryId)->get();
    }

    #[Computed]
    public function totalPerProductPaginated()
    {
        // Dummy pagination data for product summary table
        return [
            'currentPage' => 1,
            'lastPage' => 1,
    ...
    {
        // Method for product summary pagination
    ...
    public function render()
    {
        ...
        // INCLUDE both aktual and hold batches to show all stock
        $query = StockBatch::where('qty', '>', 0)
            ->whereIn('status', ['aktual', 'hold']) // Show both actual and hold stock
            ->with(['product', 'product.category', 'product.subcategory', 'category', 'subcategory'])
        ...
        // Include both aktual and hold batches
        $productNumberQuery = StockBatch::where('qty', '>', 0)
            ->whereIn('status', ['aktual', 'hold'])
            ->join('products', 'stock_batches.product_id', '=', 'products.id')
    ...
    public function deleteBatch($batchId)
    {
        try {
            ...
            // Delete related StockCard entries (audit trail)
            StockCard::where('batch_id', $batch->id)->delete();

            // Delete the batch itself
            $batch->delete();

            $this->dispatch('notify', message: 'Batch dan data terkaitnya berhasil dihapus!', type: 'success');
    ...
    {
        try {
            $batch = StockBatch::with('product', 'category', 'subcategory')
                ->findOrFail($batchId);

            $this->editBatchId = $batch->id;
            $this->editProductId = $batch->product_id;
            // Use batch's category/subcategory, fallback to product's if not set
            $this->editCategoryId = $batch->category_id ?? $batch->product->category_id ?? null;
            $this->editSubcategoryId = $batch->subcategory_id ?? $batch->product->subcategory_id ?? null;
    ...
    public function updateBatch()
    {
        ...
        try {
            ...
            // Update batch with category and subcategory
            $batch->update([
                'nama_tumpukan' => $this->editNamaTumpukan,
                'qty' => $this->editQty,
                'note' => $this->editNote,
                'category_id' => $this->editCategoryId,
                'subcategory_id' => $this->editSubcategoryId,
            ]);

            // Update satuan di product (only satuan, not category/subcategory)
            if ($batch->product) {
    ...
    public function deleteSelected()
    {
        ...
        try {
            ...
            // Delete related StockCard entries (audit trail) for all selected batches
            StockCard::whereIn('batch_id', $batchIds)->delete();
            ...
            if ($count > 0) {
                ...
                session()->flash('message', "$count batch dan data terkaitnya berhasil dihapus!");
            } else {
    ...
    public function quickAddProduct()
    {
        ...
        try {
            ...
            // Set product search value for display - use nama produk untuk display
            $this->createProductSearch = $product->nama_produk;

            // Set category dan subcategory
            $this->createCategoryId = $product->category_id;
            $this->createSubcategoryId = $product->subcategory_id;
            $this->createSatuan = $product->satuan ?? '';
            ...
            // Dispatch event to update product list globally
