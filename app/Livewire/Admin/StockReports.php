...
class StockReports extends Component
{
    ...
    public function render()
    {
        ...
        // Get category and subcategory breakdown
        $stokByCategory = $this->getStokByCategory($this->activeTab === 'store' ? 'store' : 'warehouse');
        $stokBySubCategory = $this->getStokBySubCategory($this->activeTab === 'store' ? 'store' : 'warehouse');

        return view('livewire.admin.stock.stock-reports', [
            'stocks' => $stocks,
            'batchTotals' => $batchTotals,
            'adjustments' => $this->adjustments,
            'holdTotals' => $holdTotals,
            'stokByCategory' => $stokByCategory,
            'stokBySubCategory' => $stokBySubCategory,
        ]);
    }
    ...
    public function getStokByCategory($locationType = 'store')
    {
        $query = StockBatch::with(['product.category', 'product.subcategory'])
            ->where('location_type', $locationType);

        if ($locationType === 'store') {
            $query->where('location_id', $this->filterStoreId);
        } else {
            $query->where('location_id', $this->filterWarehouseId);
        }
    ...
    public function getStokBySubCategory($locationType = 'store')
    {
        $query = StockBatch::with(['product.category', 'product.subcategory'])
            ->where('location_type', $locationType);
        ...
        return $query->get()
            ...
            ->map(function ($batches) {
                ...
                return [
                    ...
                    'subcategory_name' => $subcategory?->nama_subkategori ?? 'Tanpa Subkategori',
                    'category_name' => $category?->nama_kategori ?? 'Tanpa Kategori',
                    'total_qty' => $batches->sum('qty'),
                    'product_count' => $products->count(),
                    'products' => $products->map(fn ($p) => $p?->nama_produk ?? 'Unknown')->implode(', '),
